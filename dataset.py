...
def get_multihead_loader(datasets, local_rank, batch_size, seed = 2048, num_workers = 8,) -> Iterable:
    transform = transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
    ...
    # multihead data loader
    train_set = multihead_data(datasets, transform)
    ...
    return train_loader, data_size

class multihead_data(data.Dataset):
    def __init__(self, datasets, tensor_transform):
        self.transforms = tensor_transform
        # set raw image directory as img root
        self.head_num = len(datasets)
        self.dataset=[[] for i in range(self.head_num)]
        self.data_size = []

        # for each dataset
        for dataset_idx, dataset in enumerate(datasets):
            assert os.path.exists(dataset), f"root: {dataset} not found."

            subj_list = os.listdir(dataset) # for debug
            self.data_size.append(len(subj_list))
            for subj_idx, subj in enumerate(subj_list):
                subj_path = os.path.join(dataset, subj)
                file_list = os.listdir(subj_path)
                for file in file_list:
                    file_path = os.path.join(subj_path, file)
                    
                    self.dataset[dataset_idx].append((file_path, subj_idx)) # image path, label
        


    def __len__(self):
        # largest dataset
        return len(max(self.dataset, key=len))
    ...
    def __getitem__(self, index):
        ...
        for dataset in self.dataset:
            img_path, label = dataset[index % len(dataset)]
...
class MultiLoaderX(DataLoader):
    def __init__(self, local_rank, **kwargs):
        super(MultiLoaderX, self).__init__(**kwargs)
        self.stream = torch.cuda.Stream(local_rank)
        self.local_rank = local_rank

    def __iter__(self):
        self.iter = super(MultiLoaderX, self).__iter__()
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            for data_num in range(len(self.batch[0])):
                for k in range(len(self.batch[0][data_num])):
                    # image
                    self.batch[0][data_num][k] = self.batch[0][data_num][k].to(device=self.local_rank, non_blocking=True)
                    # label
                    self.batch[1][data_num][k] = self.batch[1][data_num][k].to(device=self.local_rank, non_blocking=True)
